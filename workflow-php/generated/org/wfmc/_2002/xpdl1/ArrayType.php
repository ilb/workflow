<?php
	...
	class ArrayType extends \Happymeal\Port\Adaptor\Data\XML\Schema\AnyComplexType {
			
		const NS = "http://www.wfmc.org/2002/XPDL1.0";
		const ROOT = "ArrayType";
		const PREF = NULL;
		/**
		 * @maxOccurs 1 
		 ...
		 * @var \NMTOKEN
		 */
		protected $LowerIndex = null;
		/**
		 * @maxOccurs 1 
		 * @var \NMTOKEN
		 */
		protected $UpperIndex = null;
		public function __construct() {
			...
			$this->_properties["LowerIndex"] = array(
				"prop"=>"LowerIndex",
				"ns"=>"",
				"minOccurs"=>1,
				"text"=>$this->LowerIndex
			);
			$this->_properties["UpperIndex"] = array(
				"prop"=>"UpperIndex",
				"ns"=>"",
				"minOccurs"=>1,
				"text"=>$this->UpperIndex
			);
		}
		/**
		 ...
		 * @param \NMTOKEN $val
		 */
		public function setLowerIndex (  $val ) {
			$this->LowerIndex = $val;
			$this->_properties["LowerIndex"]["text"] = $val;
		}
		/**
		 * @param \NMTOKEN $val
		 */
		public function setUpperIndex (  $val ) {
			$this->UpperIndex = $val;
			$this->_properties["UpperIndex"]["text"] = $val;
		}
		/**
		...
		*/
		protected function attributesToXmlWriter ( \XMLWriter &$xw, $xmlname = self::ROOT, $xmlns = self::NS ) {
			...
			if( $prop = $this->getLowerIndex() ) $xw->writeAttribute( 'LowerIndex', $prop );
			if( $prop = $this->getUpperIndex() ) $xw->writeAttribute( 'UpperIndex', $prop );
		}
		/**
		...
		*/
		...
		public function attributesFromXmlReader ( \XMLReader &$xr ) {
			if( $attr = $xr->getAttribute( 'LowerIndex') ) {
			$this->_attributes['LowerIndex']['prop'] = 'LowerIndex';
			$this->setLowerIndex( $attr ); 
		}
			if( $attr = $xr->getAttribute( 'UpperIndex') ) {
			$this->_attributes['UpperIndex']['prop'] = 'UpperIndex';
			$this->setUpperIndex( $attr ); 
		}
		...
		public function fromJSON( $arg ) {
			...
			if(isset($props["LowerIndex"])) {
				$this->setLowerIndex($props["LowerIndex"]);
			}
			if(isset($props["UpperIndex"])) {
				$this->setUpperIndex($props["UpperIndex"]);
			}
		}
