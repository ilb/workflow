...
class DeadlineInstance extends \Happymeal\Port\Adaptor\Data\XML\Schema\AnyComplexType {
    ...
    protected function attributesToXmlWriter(\XMLWriter &$xw, $xmlname = self::ROOT, $xmlns = self::NS) {
        ...
        if ($prop = $this->getIsExecuted())
            $xw->writeAttribute('IsExecuted', $prop);
        if ($prop = $this->getIsSynchronous())
            $xw->writeAttribute('IsSynchronous', $prop);
        if ($prop = $this->getExceptionName())
            $xw->writeAttribute('ExceptionName', $prop);
        if ($prop = $this->getTimeLimit())
            $xw->writeAttribute('TimeLimit', $prop);
    }
