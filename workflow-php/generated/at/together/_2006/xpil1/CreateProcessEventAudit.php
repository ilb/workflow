...
class CreateProcessEventAudit extends \at\together\_2006\xpil1\EventAudit {
    ...
    protected function attributesToXmlWriter(\XMLWriter &$xw, $xmlname = self::ROOT, $xmlns = self::NS) {
        ...
        if ($prop = $this->getPWorkflowProcessFactoryId())
            $xw->writeAttribute('PWorkflowProcessFactoryId', $prop);
        if ($prop = $this->getPWorkflowProcessFactoryVersion())
            $xw->writeAttribute('PWorkflowProcessFactoryVersion', $prop);
        if ($prop = $this->getPWorkflowProcessInstanceId())
            $xw->writeAttribute('PWorkflowProcessInstanceId', $prop);
        if ($prop = $this->getPWorkflowProcessInstanceName())
            $xw->writeAttribute('PWorkflowProcessInstanceName', $prop);
        if ($prop = $this->getPActivityInstanceId())
            $xw->writeAttribute('PActivityInstanceId', $prop);
        if ($prop = $this->getPPackageId())
            $xw->writeAttribute('PPackageId', $prop);
        if ($prop = $this->getPProcessDefinitionId())
            $xw->writeAttribute('PProcessDefinitionId', $prop);
        if ($prop = $this->getPActivityDefinitionId())
            $xw->writeAttribute('PActivityDefinitionId', $prop);
    }
