...
class ExecutionInstance extends \Happymeal\Port\Adaptor\Data\XML\Schema\AnyComplexType {
    ...
    protected function attributesToXmlWriter(\XMLWriter &$xw, $xmlname = self::ROOT, $xmlns = self::NS) {
        ...
        if ($prop = $this->getDefinitionId())
            $xw->writeAttribute('DefinitionId', $prop);
        if ($prop = $this->getName())
            $xw->writeAttribute('Name', $prop);
        if ($prop = $this->getState())
            $xw->writeAttribute('State', $prop);
        if ($prop = $this->getCreated())
            $xw->writeAttribute('Created', $prop);
        if ($prop = $this->getStarted())
            $xw->writeAttribute('Started', $prop);
        if ($prop = $this->getFinished())
            $xw->writeAttribute('Finished', $prop);
    }
